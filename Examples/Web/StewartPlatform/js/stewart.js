...
(function (root) {
  ...
  function parseSVGPath(str) {
    var p = str.match(/[a-z]|[-+]?([0-9]*\.[0-9]+|[0-9]+)/gi);
  ...
  Animation.SVG = function (svg, box) {
    ...
    function move(x, y, z) {
      ...
      var relCurX =
        ((cur.x - box.x) / box.width) * SCREEN_SIZE - SCREEN_SIZE / 2;
      var relCurY =
        ((cur.y - box.y) / box.height) * SCREEN_SIZE - SCREEN_SIZE / 2;
      ...
      ret.push({
        ...
        y: relY,
        z: z,
        t: Math.hypot(relX - relCurX, relY - relCurY, z - cur.z) / PERSEC,
      });
    ...
    for (var i = 0; i < seg.length; i++) {
      ...
      switch (s.cmd) {
        ...
        case "arc":
          ...
          // Step 1: x1', y1'
          var x1_ =
            (Math.cos(axisRotation) * (x1 - x2)) / 2.0 +
            (Math.sin(axisRotation) * (y1 - y2)) / 2.0;
          var y1_ =
            (-Math.sin(axisRotation) * (x1 - x2)) / 2.0 +
            (Math.cos(axisRotation) * (y1 - y2)) / 2.0;
          ...
          var cx =
            (x1 + x2) / 2.0 +
            Math.cos(axisRotation) * cx_ -
            Math.sin(axisRotation) * cy_;
          var cy =
            (y1 + y2) / 2.0 +
            Math.sin(axisRotation) * cx_ +
            Math.cos(axisRotation) * cy_;
          ...
          // initial angle
          var theta1 = angleBetween(1, 0, (x1_ - cx_) / rx, (y1_ - cy_) / ry);
  ...
  Animation.prototype = {
    ...
    drawPath: function (p) {
      if (!this.pathVisible || !this.cur.pathVisible) return;
      ...
      for (var i = 0; i <= steps; i++) {
        ...
        p.vertex(
          this.translation[0],
          this.translation[1],
          this.translation[2] + this.platform.T0[2]
        );
      }
      p.endShape();
    },
    ...
    fn: {
      ...
      breathe: {
        ...
        fn: function (pct) {
          var y =
            Math.exp(Math.sin(2 * Math.PI * pct) - 1) / (Math.E * Math.E - 1);
      ...
      lissajous: {
        ...
        fn: function (pct) {
          this.translation = [
            Math.sin(3 * pct * 2 * Math.PI) * 30,
            Math.sin(pct * 2 * 2 * Math.PI) * 30,
            0,
          ];
          this.orientation = Quaternion.ONE;
        },
      ...
      helical: {
        ...
        fn: function (pct) {
          pct = 1 - pct;
          this.translation = [
            Math.cos(pct * Math.PI * 8) * 20,
            Math.sin(pct * Math.PI * 8) * 20,
            pct * 20,
          ];
          this.orientation = Quaternion.ONE;
        },
        ...
        return {
          ...
          start: function () {
            ...
            if (gamepadActive) {
              ...
              alert(
                "Plug in a Playstation or Xbox controller and use the joysticks"
              );
            }
          },
          fn: function () {
            ...
            if (buttons[6].value) {
              ...
              this.orientation = Quaternion.fromAxisAngle(
                [0, 0, 1],
                (-axes[3] * Math.PI) / 6
              );
              this.translation = [0, 0, 0];
  ...
  Stewart.prototype = {
    ...
    init: function (opts) {
      ...
      if (opts.absoluteHeight) {
        ...
        this.T0 = [
          ...
          0,
          Math.sqrt(
            this.rodLength * this.rodLength +
              this.hornLength * this.hornLength -
    ...
    initCircular: function (opts) {
      ...
      var servoRangeVisible =
        opts.servoRangeVisible === undefined ? false : opts.servoRangeVisible;
      ...
      this.init({
        ...
        getLegs: function () {
          ...
          for (var i = 0; i < 6; i++) {
            ...
            legs.push({
              baseJoint: [
                Math.cos(phiB) * baseRadius,
                Math.sin(phiB) * baseRadius,
                0,
              ],
              platformJoint: [
                Math.cos(phiP) * platformRadius,
                Math.sin(phiP) * platformRadius,
                0,
              ],
              motorRotation:
                phiB + ((i + hornDirection) % 2) * Math.PI + Math.PI / 2,
            });
          }
    ...
    initHexagonal: function (opts) {
      ...
      var platformTurn =
        opts.platformTurn === undefined ? true : opts.platformTurn;
      ...
      var platformInts = getHexPlate(
        platformRadius,
        platformRadiusOuter,
        platformTurn ? Math.PI : 0
      );
      ...
      this.init({
        ...
        getLegs: function () {
          ...
          for (var i = 0; i < 6; i++) {
            ...
            basePoints.push([
              baseMidX + baseDX * shaftDistance * pm,
              baseMidY + baseDY * shaftDistance * pm,
              0,
            ]);
            platPoints.push([
              platMidX + baseDX * anchorDistance * pm,
              platMidY + baseDY * anchorDistance * pm,
              0,
            ]);
            motorAngle.push(
              Math.atan2(baseDY, baseDX) + ((i + hornDirection) % 2) * Math.PI
            );
          }
      ...
      return function (p) {
        ...
        for (var i = 0; i < this.B.length; i++) {
          ...
          if (this.servoRangeVisible) {
            ...
            p.rotateY(
              Math.atan2(
                this.H[i][1] - this.B[i][1],
                this.H[i][0] - this.B[i][0]
              )
            );
    ...
    update: function (translation, orientation) {
      ...
      for (var i = 0; i < B.length; i++) {
        ...
        var fk =
          2 * hornLength * (this.cosBeta[i] * li[0] + this.sinBeta[i] * li[1]);
    ...
    getServoAngles: function (inDegrees = true) {
      ...
      for (var i = 0; i < this.B.length; i++) {
        ...
        if (isNaN(ret[i])) {
          ...
          !(this.servoRange[0] <= ret[i] && ret[i] <= this.servoRange[1])
        ) {
          // Out of range
          ret[i] = null;
        }
